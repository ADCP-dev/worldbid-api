...
#### Step 6c2: E2E Tests with Playwright (Real Execution)

For changes touching frontend code (Nuxt/Vue), run Playwright E2E tests:

```
Check if Playwright is configured:
├── playwright.config.ts exists? → YES
├── @playwright/test in package.json? → YES  
├── tests/e2e/ directory exists? → YES
└── Run: npx playwright test

Execute: cd apps/front && npx playwright test
Capture:
├── Test count: total / passed / failed
├── Failures: list of failed test names + error messages
└── Screenshots: note if any were captured

Flag: CRITICAL if any E2E test fails AND the failure is related to the change
Flag: WARNING if tests fail but are pre-existing (not from this change)
```

If no Playwright config exists, skip this step (not a CRITICAL failure).
