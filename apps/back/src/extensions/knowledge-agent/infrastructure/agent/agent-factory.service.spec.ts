...
describe('AgentFactoryService', () => {
  ...
  it('should pass native + MCP tools merged in the tools array', async () => {
    agentConfigRepo.findById.mockResolvedValue(makeConfig());
    const native = [makeTool('search_notes'), makeTool('execute')];
    ...
    expect((captured.tools as Array<{ name: string }>).map((t) => t.name)).toEqual(
      expect.arrayContaining(['search_notes', 'execute', 'get_weather']),
    );
  });
  ...
  it('should cache: same config → same agent (no rebuild)', async () => {
    const cfg = makeConfig();
    agentConfigRepo.findById.mockResolvedValue(cfg);
    toolRegistry.collect.mockResolvedValue([]);
    mcpLoader.load.mockResolvedValue([]);
