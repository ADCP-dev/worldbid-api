 ...
 *   4. Cache miss → collect native tools + MCP tools, create a VfsBackend,
 *      call `createDeepAgent({ model, systemPrompt, tools, backend, permissions })`,
 *      cache and return.
 *
 * Cache invalidation: any change to systemPrompt, model, permissions, or
 * mcpServerIds produces a new hash → rebuild on next call.
 */
@Injectable()
export class AgentFactoryService {
  ...
  async buildAgent(configId: string, userId: number): Promise<unknown> {
    ...
    if (cached && cached.hash === hash) {
      this.logger.debug(`Cache hit for agent ${configId}`);
      return cached.agent;
    }

    this.logger.log(`Building agent ${configId} (cache miss)`);
    const agent = await this.constructAgent(config);
    this.cache.set(configId, { agent, hash });
    return agent;
  }

  private async constructAgent(config: AgentConfig): Promise<unknown> {
    ...
    const tools: StructuredTool[] = [...nativeTools, ...mcpTools];
