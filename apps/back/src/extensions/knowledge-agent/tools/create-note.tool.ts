 ...
 *
 * Factory: receives the NoteService + userId. The userId closure enforces
 * scoping — the created note belongs to the requesting user. On success the
 * NoteService enqueues an async embedding job (Bull) so `create` returns
 * immediately with the note id (Q-13: always async).
 */
export function createCreateNoteTool(noteService: NoteService, userId: number) {
  return tool(
    ...
    },
    {
      name: 'create_note',
      description:
        'Create a new note in the knowledge base. The note is owned by the current user and an embedding is generated asynchronously.',
      schema: z.object({
