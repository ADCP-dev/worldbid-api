...
<template>
  <div>
    ...
    <Teleport to="body">
        ...
        >
            <div class="absolute -top-6 left-[-2px] bg-primary text-primary-content text-xs px-2 py-0.5 rounded-t-sm whitespace-nowrap font-mono shadow-sm">
                {{ hoveredKey }}
            </div>
        </div>
    </Teleport>
    ...
    <dialog class="modal" :class="{ 'modal-open': isEditing }">
      <div class="modal-box max-w-xl max-h-[80vh] flex flex-col p-6">
        ...
        <div class="flex-1 overflow-y-auto pr-2 flex flex-col gap-4 mb-4">
          <div v-for="lang in allLangs" :key="lang.id" class="card card-compact border border-base-content/20 bg-base-100">
            <div class="card-body">
