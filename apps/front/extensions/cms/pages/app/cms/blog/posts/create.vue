...
const handleCoverUpload = async (event: Event) => {
  ...
  try {
    ...
    const response = await fetch(
      `${useRuntimeConfig().public.apiUrl}/api/v1/cms/media/upload`,
      {
        method: "POST",
        headers: {
    ...
    featuredImage.value = { id: result.id, url: result.url };
    form.value.featuredImageId = result.id;
...
<template>
  <div class="container mx-auto py-8 max-w-7xl">
    ...
    <div class="flex justify-between items-center mb-8">
      ...
      <button type="submit" class="btn btn-primary" :disabled="loading || isCreating" @click="handleSubmit">
      ...
      <div class="card bg-base-100 shadow-sm border">
        <div class="card-body">
          ...
          <p class="text-sm text-base-content/60 mb-4">
            Las traducciones se podrán gestionar después de guardar el post.
          </p>
        </div>
      </div>
