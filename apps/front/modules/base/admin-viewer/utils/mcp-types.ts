/**
 * MCP view type mirrors for the frontend admin-viewer layer.
 *
 * These mirror the backend `apps/back/src/mcp/types.ts` shapes so the UI
 * has full typing without importing backend source. Keep in sync with
 * backend types when they change.
 */
...
export interface ErrorView {
  ...
  stack?: string;
  createdAt?: string;
  suggestedFixes?: SuggestedFixView[];
}

export interface SuggestedFixView {
  type: string;
  confidence: string;
  description?: string;
  target?: string;
  field?: string;
  changes?: Array<{ file: string; type: string; content: string }>;
}
...
export interface FrontendLayerView {
  ...
  confidence: string;
  fixType: string;
  changes: Array<{ file: string; before: string; after: string; diff: string }>;
  prUrl: string | null;
  reason: string | null;
  createdAt: string;
}
