...
4. **Tools nativas** — auto-discovery via `agent.tools.ts` por extensión. `ToolRegistryService` hace `fs.readdirSync(extensionsDir)` + dynamic import, graceful skip si no existe. Tools: `search_notes_tree`, `search_notes_semantic`, `create_note`, `update_note`, `delete_note`, `execute`.
